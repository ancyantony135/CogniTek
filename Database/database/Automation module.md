...
## 2. Your Mission: The KTU Scraper
    1. Cognitek needs to know when exams or results are out without the student checking the portal.
    
    2. Target: The KTU (Kerala Technological University) portal or student login.

    3. Goal: Scrape "Upcoming Exams" or "Internal Marks" and convert them into Tasks in our shared cognitek.db.

    4. Integration: You must use the SQLAlchemy models defined in main.py so the data you scrape shows up on Elvin's frontend immediately.
...
Since we are building a student assistant, you need to help the AI (Gemini) create study materials.
    ...
    ➡️ Task: Write a script that takes a PDF (Syllabus) and breaks it into "Topics to Study."

    ➡️ Output: Send these topics to the api/chat endpoint so Gemini can generate flashcards for the student.
...
Database Connection: 
    ...
    Use the relative path sqlite:///../Database/database/cognitek.db to save the data you scrape. 
This ensures we all share the same memory.
